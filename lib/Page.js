...
function Page(pageConfig) {
  ...
  this.includedTags = pageConfig.includedTags;
...
/**
 * Filters out elements on the page based on included tags
 * @param content of the page
 * @param tags to include
 */
Page.prototype.filterTags = function (includedTags, content) {
  if (!includedTags) {
    return content;
  }
  ...
  Object.keys(includedTags).forEach((tagType) => {
    includedTags[tagType].forEach((tag) => {
      const includedTag = `${tagType}--${tag}`;
      $(`[tag="${includedTag}"]`).each((i, element) => {
...
Page.prototype.collectFrontMatter = function (includedPage) {
  ...
  if (frontMatter.length) {
    ...
    this.frontMatter.includedTags = (this.frontMatter.include || this.includedTags || {});
  } else {
