...
function calculateBoilerplateFilePath(filePath, config) {
  const { parent, relative } = calculateNewBaseUrl(filePath, config.rootPath, config.baseUrlMap);
  return path.resolve(parent, relative, BOILERPLATE_DIRECTORY_NAME, path.basename(filePath));
}
...
Parser.prototype.includeFile = function (file, config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(actualFilePath, 'utf-8', (err, data) => {
      ...
      const { parent, relative } = calculateNewBaseUrl(file, config.rootPath, config.baseUrlMap);
      const userDefinedVariables = config.userDefinedVariablesMap[path.resolve(parent, relative)];
